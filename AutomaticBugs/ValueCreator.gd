...
# Only generate valid values e.g. Use Node2D instead of Node
var should_be_always_valid: bool = true
...
func get_object(object_name: String) -> Object:
	assert(ClassDB.class_exists(object_name), "Class " + object_name + " doesn't exist.")
	...
	if random:
			...
			if should_be_always_valid:
				...
				while true:
					...
					if a > 50:
						...
						# This shouldn't happen, but does happen e.g. SpatialGizmo.
		...
		# Note: Texture, for example, is not instantable (or shouldn't be) but LargeTexture is.
		if ClassDB.can_instance(object_name):
			...
			# Number of inherited class of non instantable classes must be greater than 0.
